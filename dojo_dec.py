...
for word in word_couplings.keys():
    for row in rows_in:
        ...
        for i in range(0, len(sentence)-1):
            for item in punctuation:
                if item in sentence[i]:
                    sentence[i] = sentence[i].replace(item, '')
            if sentence[i].lower() == word:
                for item in punctuation:
                    if item in sentence[i+1]:
                        sentence[i+1] = sentence[i+1].replace(item, '')
                word_couplings[word].append(sentence[i+1].lower())

print 'The length of the file is ', len(rows_in), 'sentences'
